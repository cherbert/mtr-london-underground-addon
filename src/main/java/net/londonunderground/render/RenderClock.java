...
import net.londonunderground.data.IGui;
...
public class RenderClock extends BlockEntityRenderer<LUClock.TileEntityClock> implements IGui, IBlock {
	...
	@Override
	public void render(LUClock.TileEntityClock entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
	...
	private static void drawHand(MatrixStack matrices, VertexConsumerProvider vertexConsumers, float rotation, boolean isHourHand) {
		...
		IDrawing.drawTexture(matrices, vertexConsumer, -0.01F, isHourHand ? 0.15F : 0.24F, isHourHand ? 0.1F : 0.105F, 0.01F, -0.03F, isHourHand ? 0.1F : 0.105F, Direction.UP, ARGB_RED, MAX_LIGHT_INTERIOR);
		matrices.pop();
	}
}
